...
def payload_for(spec,prompt):
    ...
    for x in spec.get('parameters',[]):
        ...
        elif l in {'system','system_prompt'}: p[n]='CLAIM<=EVIDENCE; SIMULATION!=TEST; VERIFY BEFORE COMMIT; UNKNOWN REMAINS UNKNOWN.'
...
def invoke(space,prompt):
    ...
    except Exception as e: return False,None,{'error':f'INFO_PARSE_FAILED:{e!r}'}
